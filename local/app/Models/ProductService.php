...
use App\Models\CreatedProductService;
use App\Models\UpdatedProductService;
use App\Models\DeletedProductService;
use Request;
use App\Models\ProductServiceStatus;
use App\Models\ProductServiceField;
class ProductService extends Model
{
	protected $table = 'ap_product_service';
	...
	public function __create__(){
		...
		/******/
		/*your code*/
		$complement = new CreatedProductService;
	...
	public function __update__(){
		...
		/******/
		/*your code*/
		$complement = new UpdatedProductService;
		$complement->id_item = $this->id;
		$complement->info = $info;
		$complement->hash_operation = HASH_OPERATION;
	...
	public function __delete__(){
		...
		/******/
		/*your code*/
		$complement = new DeletedProductService;
		$complement->id_item = $this->id;
		$complement->info = $info;
		$complement->hash_operation = HASH_OPERATION;
	...
	public function read_Status($args = array()){
		return $this->hasMany('App\Models\ProductServiceStatus', 'id_item');
	}
	...
	public function create_Field($args){
		if(gettype($args) == "object"){
			...
			if(isAssoc($args)){
				$obj = new ProductServiceField;
				foreach ($args as $field => $value){
					$obj->$field = $value;
				}
				$obj->id_product_service = $this->id;
				return $obj->__create__();
			}else{
				$ret = array();
				foreach ($args as $key => $item) {
	...
	public function read_Field($args = array()){
		return $this->hasMany('App\Models\ProductServiceField', 'id_product_service');
	}
	public function update_Field($args = array()){
	}
	public function delete_Field($args = array()){
		$list = $this->read_Field;
		foreach($list as $item){
			$item->__delete__();
		}
