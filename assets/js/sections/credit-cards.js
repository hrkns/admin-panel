...
/**/
	this.FLAGS = {};
	this.permises = {};
	this.ENDPOINT_ITEMS_INDEX = "/credit-cards";
	this.ENDPOINT_ITEMS_SEARCH = "/credit-cards-search";
	this.ENDPOINT_ITEM = "/credit-card/";
	this.getItems = null;
	...
	this.add_item_form_to_dom = function(data, configs){
		...
		function watcher(tkn){
			if(!($(row).attr('data-updating') == '1' || $(row).attr('data-deleting') == '1' || $(row).attr('data-getting') == '1')){
			...
			if(Section.permises["update"]){
				if(App.FORMAT_EDIT_ITEMS == "inline"){
					...
					button_get_edit_view.onclick = function(){
						App.getView("credit-card", "edit", function(){
							var DOM_form_code = $("#modal_credit-card_edit").find("input[name='code']");
							DOM_form_code.val(data.code);

							var DOM_form_name = $("#modal_credit-card_edit").find("input[name='name']");
							DOM_form_name.val(data.name);

							var DOM_form_description = $("#modal_credit-card_edit").find("textarea[name='description']");
							DOM_form_description.val(data.description);

							var DOM_form_statuses = $("#modal_credit-card_edit").find("select[name='status']");
							...
							$("#form_credit-card_edit").submit(function(e){
								e.preventDefault();
								...
								App.DOM_Disabling("#modal_credit-card_edit");

								App.HTTP.update({
									url : App.WEB_ROOT + "/credit-card/" + data.id,
									data : {
										lng : configs.lng,
										name : name, 
									...
									success : function(){
										...
										$("#modal_credit-card_edit").modal("hide");
									}, error : function(){
									}, after : function(){
										App.HideLoading();
										App.UnlockScreen();
										App.DOM_Enabling("#modal_credit-card_edit");
									}
								});
							});
						});
