...
/**/
	this.FLAGS = {};
	this.permises = {};
	this.ENDPOINT_ITEMS_INDEX = "/media";
	this.ENDPOINT_ITEMS_SEARCH = "/media-search";
	this.ENDPOINT_ITEM = "/media/";
	this.getItems = null;
	...
	this.add_item_form_to_dom = function(data, configs){
		...
		function watcher(tkn){
			var name = input_name;
			var descr = input_description;

			if(!($(row).attr('data-getting') == '1' || $(row).attr('data-deleting') == '1' || $(row).attr('data-updating') == '1')){
		    	if(input_code.value.trim().length > 0 && name.value.trim().length > 0 && (input_code.value.trim() != data.code || name.value.trim() != data.name || $(descr).val().trim() != data.description || !App.flexible_equal_array($(select_statuses).val(), data.status))){
			...
			if(Section.permises["update"]){
				if(App.FORMAT_EDIT_ITEMS == "inline"){
					...
					button_get_edit_view.onclick = function(){
						App.getView("media", "edit", function(){
							var DOM_form_code = $("#modal_media_edit").find("input[name='code']");
							DOM_form_code.val(data.code);

							var DOM_form_name = $("#modal_media_edit").find("input[name='name']");
							DOM_form_name.val(data.name);

							var DOM_form_description = $("#modal_media_edit").find("textarea[name='description']");
							DOM_form_description.val(data.description);

							var DOM_form_statuses = $("#modal_media_edit").find("select[name='status']");
							...
							$("#form_media_edit").submit(function(e){
								e.preventDefault();
								...
								App.DOM_Disabling("#modal_media_edit");

								App.HTTP.update({
									url : App.WEB_ROOT + "/media/" + data.id,
									data : {
										lng : configs.lng,
										code : code, 
										name : name, 
									...
									success : function(){
										...
										$("#modal_media_edit").modal("hide");
									}, error : function(){
									}, after : function(){
										App.HideLoading();
										App.UnlockScreen();
										App.DOM_Enabling("#modal_media_edit");
									}
								});
							});
						});
