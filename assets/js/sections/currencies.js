...
/**/
	this.FLAGS = {};
	this.permises = {};
	this.ENDPOINT_ITEMS_INDEX = "/currencies";
	this.ENDPOINT_ITEMS_SEARCH = "/currencies-search";
	this.ENDPOINT_ITEM = "/currency/";
	this.getItems = null;
			...
			if(Section.permises["update"]){
				if(App.FORMAT_EDIT_ITEMS == "inline"){
					...
					button_get_edit_view.onclick = function(){
						App.getView("currency", "edit", function(){
							var DOM_form_code = $("#modal_currency_edit").find("input[name='code']");
							DOM_form_code.val(data.code);

							var DOM_form_name = $("#modal_currency_edit").find("input[name='name']");
							DOM_form_name.val(data.name);

							var DOM_form_description = $("#modal_currency_edit").find("textarea[name='description']");
							DOM_form_description.val(data.description);

							var DOM_form_statuses = $("#modal_currency_edit").find("select[name='status']");
							...
							$("#form_currency_edit").submit(function(e){
								e.preventDefault();
								...
								App.DOM_Disabling("#modal_currency_edit");

								App.HTTP.update({
									url : App.WEB_ROOT + "/currency/" + data.id,
									data : {
										lng : configs.lng,
										code : code, 
										name : name, 
									...
									success : function(){
										...
										$("#modal_currency_edit").modal("hide");
									}, error : function(){
									}, after : function(){
										App.HideLoading();
										App.UnlockScreen();
										App.DOM_Enabling("#modal_currency_edit");
									}
								});
							});
						});
	...
	this.generateCurrenciesSelect = function(){
		var str = "";

		$.each(Section.Currencies_byCode, function(k, item){
	...
	this.getCurrencyAttr = function(attr, by, val, transform1, transform2){
		var retornar;

		val = String(val);

		if(transform1){
			val = transform1 = "low"?val.toLowerCase():val.toUpperCase();
		}

		if(by == "code"){
			retornar = Section.Currencies_byCode[val][attr];
		}else{
			retornar = Section.Currencies_byID[val][attr];
		}

		if(transform2){
			retornar = transform2 = "low"?retornar.toLowerCase():retornar.toUpperCase();
		}

		return retornar.trim();
	}

	this.Currencies_byCode = {};
	this.Currencies_byID = {};

	this.start = function(){
		function getCurrencies(){
			App.GetMasterData("currencies", function(d, e, f){
				$.each(d.data.items, function(k, v){
					Section.Currencies_byID[String(v.id)] = v;
					Section.Currencies_byCode[String(v.code).toLowerCase()] = v;
				});
				App.HideLoading();
