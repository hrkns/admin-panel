...
/**/
	this.FLAGS = {};
	this.permises = {};
	this.ENDPOINT_ITEMS_INDEX = "/e-payment-methods";
	this.ENDPOINT_ITEMS_SEARCH = "/e-payment-methods-search";
	this.ENDPOINT_ITEM = "/e-payment/";
	this.getItems = null;
			...
			if(Section.permises["update"]){
				if(App.FORMAT_EDIT_ITEMS == "inline"){
					...
					button_get_edit_view.onclick = function(){
						App.getView("e-payment", "edit", function(){
							var DOM_form_code = $("#modal_e-payment_edit").find("input[name='code']");
							DOM_form_code.val(data.code);

							var DOM_form_name = $("#modal_e-payment_edit").find("input[name='name']");
							DOM_form_name.val(data.name);

							var DOM_form_description = $("#modal_e-payment_edit").find("textarea[name='description']");
							DOM_form_description.val(data.description);

							var DOM_form_statuses = $("#modal_e-payment_edit").find("select[name='status']");
							...
							$("#form_e-payment_edit").submit(function(e){
								e.preventDefault();
								...
								App.DOM_Disabling("#modal_e-payment_edit");

								App.HTTP.update({
									url : App.WEB_ROOT + "/e-payment/" + data.id,
									data : {
										lng : configs.lng,
										code : code, 
										name : name, 
									...
									success : function(){
										...
										$("#modal_e-payment_edit").modal("hide");
									}, error : function(){
									}, after : function(){
										App.HideLoading();
										App.UnlockScreen();
										App.DOM_Enabling("#modal_e-payment_edit");
									}
								});
							});
						});
