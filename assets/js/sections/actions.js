...
/**/
	this.FLAGS = {};
	this.permises = {};
	this.ENDPOINT_ITEMS_INDEX = "/actions";
	this.ENDPOINT_ITEMS_SEARCH = "/actions-search";
	this.ENDPOINT_ITEM = "/action/";
	this.getItems = null;
			...
			if(Section.permises["update"]){
				if(App.FORMAT_EDIT_ITEMS == "inline"){
					...
					button_get_edit_view.onclick = function(){
						App.getView("action", "edit", function(){
							var DOM_form_code = $("#modal_action_edit").find("input[name='code']");
							DOM_form_code.val(data.code);

							var DOM_form_name = $("#modal_action_edit").find("input[name='name']");
							DOM_form_name.val(data.name);

							var DOM_form_description = $("#modal_action_edit").find("textarea[name='description']");
							DOM_form_description.val(data.description);

							var DOM_form_statuses = $("#modal_action_edit").find("select[name='status']");
							...
							$("#form_action_edit").submit(function(e){
								e.preventDefault();
								...
								App.DOM_Disabling("#modal_action_edit");

								App.HTTP.update({
									url : App.WEB_ROOT + "/action/" + data.id,
									data : {
										lng : configs.lng,
										code : code, 
										name : name, 
									...
									success : function(){
										...
										$("#modal_action_edit").modal("hide");
									}, error : function(){
									}, after : function(){
										App.HideLoading();
										App.UnlockScreen();
										App.DOM_Enabling("#modal_action_edit");
									}
								});
							});
						});
