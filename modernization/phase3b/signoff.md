...
- Apache profile updated to PHP 8.2 image: yes (`modernization/docker/php/Dockerfile.phase3-apache`)
- Nginx PHP-FPM profile updated to PHP 8.2 image: yes (`modernization/docker/php/Dockerfile.phase3-fpm`)
- Compose profile wiring updated to Phase 3 Apache Dockerfile: yes (`modernization/docker-compose.phase3.yml`)
...
- Historical secret-exposure remediation closure path documented: yes (`modernization/phase3b/evidence/01-secret-history-remediation-closure.md`)
...
1. `docker compose -f modernization/docker-compose.phase3.yml --profile apache config`
2. `docker compose -f modernization/docker-compose.phase3.yml --profile nginx config`
3. `docker compose -f modernization/docker-compose.phase3.yml --profile apache build web_apache`
4. `docker compose -f modernization/docker-compose.phase3.yml --profile nginx build php_fpm`
...
1. `docker compose -f modernization/docker-compose.phase3.yml --profile apache up -d --build`
2. `curl -s -o /dev/null -w "%{http_code}" http://localhost:8081`
3. `docker compose -f modernization/docker-compose.phase3.yml --profile apache exec -T web_apache php -v`
4. `docker compose -f modernization/docker-compose.phase3.yml --profile nginx up -d --build`
5. `curl -s -o /dev/null -w "%{http_code}" http://localhost:8082`
6. `docker compose -f modernization/docker-compose.phase3.yml --profile nginx exec -T php_fpm php -v`
