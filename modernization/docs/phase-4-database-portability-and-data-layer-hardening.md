...
- Added `modernization/docker-compose.phase4.yml`.
...
#### Apache profile
```bash
docker compose -f modernization/docker-compose.phase4.yml --profile apache up --build -d
```

#### Nginx profile
```bash
docker compose -f modernization/docker-compose.phase4.yml --profile nginx up --build -d
```

### 2) Run deterministic schema + seed
#### Apache profile
```bash
docker compose -f modernization/docker-compose.phase4.yml --profile apache exec -T web_apache bash -lc "cd /var/www/html/local && php artisan migrate --seed --force"
```

#### Nginx profile
```bash
docker compose -f modernization/docker-compose.phase4.yml --profile nginx exec -T php_fpm bash -lc "cd /var/www/html/local && php artisan migrate --seed --force"
```
...
```bash
docker compose -f modernization/docker-compose.phase4.yml --profile nginx ps
docker compose -f modernization/docker-compose.phase4.yml --profile nginx logs --tail=100 web_nginx
docker compose -f modernization/docker-compose.phase4.yml --profile nginx logs --tail=100 php_fpm
```
