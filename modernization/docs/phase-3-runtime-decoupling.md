...
- Added `modernization/docker-compose.phase3.yml` with profiles:
  - `apache`: Apache + mod_php stack (`web_apache`)
  - `nginx`: Nginx + PHP-FPM stack (`web_nginx` + `php_fpm`)
- Added runtime files:
  - `modernization/docker/php/Dockerfile.phase3-fpm`
  - `modernization/docker/nginx/nginx.phase3.conf`

## How To Run

### Apache profile
```bash
docker compose -f modernization/docker-compose.phase3.yml --profile apache up --build
```
...
- If restarting from Docker Desktop UI after code changes, recreate containers (`docker compose -f modernization/docker-compose.phase3.yml --profile apache up --build -d --force-recreate`) to avoid stale runtime state.

### Nginx + PHP-FPM profile
```bash
docker compose -f modernization/docker-compose.phase3.yml --profile nginx up --build
```
...
- `modernization/docker-compose.phase3.yml` now uses the same `DB_*` values for MySQL user provisioning and for PHP app runtime env, avoiding `SQLSTATE[HY000] [1045] Access denied` mismatch errors.
...
```bash
docker compose -f modernization/docker-compose.phase3.yml --profile apache down -v --remove-orphans
```
