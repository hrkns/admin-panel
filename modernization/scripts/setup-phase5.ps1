...
$composeFile = Join-Path $root "modernization/docker-compose.phase5.yml"
...
Write-Host "[startup] Checking prerequisites..."
...
if (-not (Test-Path $envPath)) {
    ...
    Write-Host "[startup] Created local/.env from .env.example"
}

Write-Host "[startup] Applying local development defaults in local/.env"
...
if ($KeepExistingSmtp) {
    Write-Host "[startup] Keeping existing SMTP values from local/.env"
} else {
    Write-Host "[startup] Applying Mailpit SMTP defaults in local/.env"
...
try {
    ...
    if ($ResetDb) {
        Write-Host "[startup] Resetting local development volumes..."
        Invoke-Checked -Action { docker compose -f $composeFile down -v --remove-orphans } -ErrorMessage "Failed to reset local development containers/volumes."
    }

    Write-Host "[startup] Starting database and maildev services..."
    Invoke-Checked -Action { docker compose -f $composeFile up -d db maildev } -ErrorMessage "Failed to start db/maildev services."

    Write-Host "[startup] Installing PHP dependencies with Composer (runtime deps only, no legacy scripts)..."
    ...
    $liveUrl = [string]::Format("http://{0}:{1}/health/live", $loopbackHost, $appPort)
    $readyUrl = [string]::Format("http://{0}:{1}/health/ready", $loopbackHost, $appPort)

    Write-Host "[startup] Running startup checks..."
    Wait-Http -Url $liveUrl -ExpectedStatus 200 -TimeoutSeconds 180
    Wait-Http -Url $readyUrl -ExpectedStatus 200 -TimeoutSeconds 180

    Write-Host ""
    Write-Host "[startup] Startup complete."
